<div id="treninzi">
	...
	<div class="izbor">
		<div class="kartica">
			<?php if ($_SESSION['loggedin'] && $_SESSION['user_level'] == 2): ?>
				<a href="./index.php?model=training&action=submit">DODAJ NOVI TRENING</a><br><br>
			<?php endif; ?>

			<?php foreach ($training as $i => $article): ?>
				<training-listing>
					<training-item>
						<br>
						<hr>
						<br>
						<thumbnail>
							<img src="<?= $article['thumb_filename'] ?>">
						</thumbnail>
						<div>
							<h2>
								<a href="./index.php?model=training&id=<?= $article['training_id'] ?>">
									<?= $article['training_title'] ?>
								</a>
							</h2>
							<p><?= $article['training_price'] ?> RSD</p>

							<?php if ($_SESSION['loggedin'] && $_SESSION['user_level'] == 2): ?>
								<div>
									<a
										href="./index.php?model=training&action=edit&id=<?= $article['training_id'] ?>">IZMENI /</a>
									<a
										href="./index.php?model=training&action=delete&id=<?= $article['training_id'] ?>">IZBRIŠI</a>
								</div>
							<?php endif; ?>
						</div>
					</training-item>
